...
const DB_HOST = "127.0.0.1:3306"; //資料庫網路位址 (127.0.0.1)
const DB_USERNAME = "root"; //資料庫連線帳號
const DB_PASSWORD = "root"; //資料庫連線密碼
const DB_NAME = "mfee14_eteam_testserver"; //指定資料庫
...
if (isset($_FILES["excel"]) && $_FILES["excel"] !== "") {
    ...
    for ($i = 2; $i <= $highestRow; $i++) {
        ...
        $uAddr =   $spreadsheet->getActiveSheet()->getCell('J' . $i)->getValue();
        $uDiscr =   $spreadsheet->getActiveSheet()->getCell('K' . $i)->getValue();
        $recycleIndex =   $spreadsheet->getActiveSheet()->getCell('L' . $i)->getValue();



        //寫入資料
        $sql = "INSERT INTO `users` 
        (`uAcco`,`uPwd`,`uName`,`uTWId`,`uBirth`,`uGender`,`uImg`,`uPhone`,`uMail`,`uAddr`,`uDiscr`,`recycleIndex`) 
        VALUES 
        (?,?,?,?,?,?,?,?,?,?,?,?)";
